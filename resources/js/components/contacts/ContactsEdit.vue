<template>
  ...
  <form class="space-y-6" v-on:submit.prevent="saveContact">
    ...
    <template
        v-for="(phone_number, index) in contact.user.phone_numbers"
        :key="phone_number.id"
    >
      <div class="grid grid-cols-3 gap-3">
        <div>
          ...
          <div class="mt-1">
            <select
                v-model="phone_number.phone_type_id"
                class="form-select block mt-1 w-full rounded-md border-gray-300 shadow-sm"
            >
    ...
    <div class="flex place-content-end mb-4">
      <div
          class="px-4 py-2 text-white bg-indigo-600 hover:bg-indigo-700 rounded-md cursor-pointer"
      >
        <button type="button" class="text-sm font-medium" @click="addAddress()">
          Add
        </button>
      </div>
    </div>
    <template v-for="(address, index) in contact.user.addresses" :key="address.id">
      <div class="grid grid-cols-3 gap-3">
        <div>
          <label for="email" class="block text-sm font-medium text-gray-700"
          >Address Line</label
...
const addPhoneNumber = async (): Promise<void> => {
  contact.value.user.phone_numbers.push({
    id: null,
    phone_type_id: "",
    phone_number: "",
  });
...
const deletePhone = async (index: number): Promise<void> => {
  contact.value.user.phone_numbers.splice(index, 1);
};

const addAddress = async (): Promise<void> => {
  contact.value.user.addresses.push({
    id: null,
    address_line: "",
    pincode: "",
  });
...
const deleteAddress = async (index: number): Promise<void> => {
  contact.value.user.addresses.splice(index, 1);
};
</script>
